  ...
  return (
    <>
      ...
      <div
        ...
        className={twMerge(
          props.className,
          'relative mb-7 flex w-11/12 flex-col rounded-xl border border-black/10 bg-white dark:bg-aws-black-jet shadow-[0_0_30px_7px] shadow-aws-gray-ice dark:shadow-black/35 md:w-10/12 lg:w-4/6 xl:w-3/6'
        )}>
        <div className="flex w-full">
          <Textarea
            key={`textarea-${props.isNewChat}`} // Add a key to force re-render
            className="m-1 bg-transparent pr-12 scrollbar-thin scrollbar-thumb-aws-gray-ice"
          ...
          <div className="relative m-2 mr-24 flex flex-wrap gap-3">
            {base64EncodedImages.map((imageFile, idx) => (
              <div key={idx} className="relative">
                <img
                  src={imageFile}
                  className="h-16 rounded border border-aws-blue-navy dark:border-aws-black-smoke"
                  onClick={() => {
        ...
        {props.canRegenerate && (
          <div className="absolute -top-14 right-0 flex space-x-2">
            {props.canContinue && !props.disabledContinue && !props.disabled && (
              <Button
                className="bg-aws-white-smoke dark:bg-aws-black-graphite p-2 text-sm"
                outlined
